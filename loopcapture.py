...
while True:
    ...
    if k%256 == 27:
        ...
        img_path2 = '/home/coe-re/Photos/With Silt/'

        
        img_name1 = f"Without_Silt_{img_counter}_{water_quant}.png"
        img_name2 = f"With_Silt_{img_counter}_{water_quant}_{silt_quant}.png"

        #when not settle
        with open (r'Silt_Experiment_Data.csv', 'a', newline='') as file:
            file_write = csv.writer(file)
            file_write.writerow([current_date_time,img_name1,img_name2,silt_quant,water_quant])
            
        cv2.imwrite(img_path1+img_name1, image1)
        cv2.imwrite(img_path2+img_name2, image2)
        
        print(f"{img_name1} and {img_name2} written!!!")
        img_counter += 1
